...
std::istream& operator>>(std::istream& stream, Ball& ball) {
    ...
    stream >> point.x >> point.y >> vector.x >> vector.y >> red >> green >> blue >> radius >> std::boolalpha >> isCollidable;

    ball = Ball(radius, Color(red, green, blue), Velocity(vector), point, isCollidable);
...
void World::show(Painter& painter) const {
    ...
    // мира
    painter.draw(topLeft, bottomRight, Color(0, 0, 0));
    painter.draw(topLeft, bottomRight, Color(1, 0, 0));
